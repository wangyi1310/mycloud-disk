	...
	go func() {
		defer func() {
			if err := recover(); err != nil {
				client.chOpen = false
				log.Log().Error("Exception while sending email: %s, queue will be reset in 10 seconds.", err)
		...
		for {
			...
			case m, ok := <-client.ch:
				if !ok {
					log.Log().Debug("Email queue closing...")
					client.chOpen = false
					return
				}
				if !open {
				...
				if err := mail.Send(s, m); err != nil {
					log.Log().Warning("Failed to send email: %s", err)
				} else {
					log.Log().Debug("Email sent.")
				}
			// 长时间没有新邮件，则关闭SMTP连接
			case <-time.After(time.Duration(client.Config.Keepalive) * time.Second):
				if open {
					if err := s.Close(); err != nil {
						log.Log().Warning("Failed to close SMTP connection: %s", err)
					}
					open = false
				}
			}
